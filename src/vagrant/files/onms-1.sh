...
if [ ! -f "/etc/pgpool-II-$pg_family/.configured" ]; then
   echo "Configuring PGPool-II ..."
   pgpool_conf=/etc/pgpool-II-$pg_family/pgpool.conf
   sudo cp /etc/pgpool-II-$pg_family/pgpool.conf.sample-stream $pgpool_conf
   sudo sed -r -i "/listen_addresses/s/localhost/*/" $pgpool_conf 
   sudo sed -r -i "/backend_hostname0/s/host1/pgdbsrv01/" $pgpool_conf 
   sudo sed -r -i "/backend_data_directory0/s/= '\/data'/= '\/var\/lib\/pgsql\/$pg_version\/data'/" $pgpool_conf
   ...
   sudo sed -r -i "s/#backend_data_directory1 = '\/data1'/backend_data_directory1 = '\/var\/lib\/pgsql\/$pg_version\/data'/" $pgpool_conf
   ...
   sudo sed -r -i "/pid_file_name/s/=.*/= '\/var\/run\/pgpool-II-$pg_family\/pgpool.pid'/" $pgpool_conf
   sudo sed -r -i "/logdir/s/=.*/= '\/var\/log\/pgpool-II-$pg_family'/" $pgpool_conf
   ...
   sudo sed -r -i "/failover_command/s/''/'\/etc\/pgpool-II-$pg_family\/failover.sh %h %H'/" $pgpool_conf
   sudo sed -r -i "/recovery_user/s/nobody/pgpool/" $pgpool_conf
   sudo sed -r -i "/recovery_password/s/''/'pgpool'/" $pgpool_conf
   sudo chmod 0600 /etc/pgpool-II-$pg_family/pgpool.conf
   sudo chown postgres:postgres /etc/pgpool-II-$pg_family/pgpool.conf

   sudo cat <<EOF > /etc/pgpool-II-$pg_family/failover.sh
#!/bin/sh 
failed_node=\$1
new_master=\$2
(
...
set -x
/usr/bin/ssh -T -l postgres \$new_master "$pg_home/bin/repmgr -f /etc/repmgr/$pg_version/repmgr.conf standby promote 2>/dev/null 1>/dev/null <&-"
exit 0;
) 2>&1 | tee -a /var/log/pgpool-II-$pg_family/pgpool_failover.log
EOF

  sudo chmod 0700 /etc/pgpool-II-$pg_family/failover.sh
  sudo chown postgres:postgres /etc/pgpool-II-$pg_family/failover.sh

  sudo cat <<EOF > /etc/pgpool-II-$pg_family/pool_hba.conf
...
EOF
  ...
  sudo chmod 0600 /etc/pgpool-II-$pg_family/pool_hba.conf
  sudo chown postgres:postgres /etc/pgpool-II-$pg_family/pool_hba.conf

  sudo touch /etc/pgpool-II-$pg_family/pool_passwd
  sudo chmod 600 /etc/pgpool-II-$pg_family/pool_passwd
  sudo chown postgres:postgres /etc/pgpool-II-$pg_family/pool_passwd
  ...
  sudo echo "postgres:"`pg_md5 postgres` >> /etc/pgpool-II-$pg_family/pcp.conf
  sudo chmod 0600 /etc/pgpool-II-$pg_family/pcp.conf
  sudo chown postgres:postgres /etc/pgpool-II-$pg_family/pcp.conf

  sudo mkdir -p /var/log/pgpool-II-$pg_family
  sudo chown postgres:postgres /var/log/pgpool-II-$pg_family

  touch /etc/pgpool-II-$pg_family/.configured
fi
...
EOF
  ...
  sudo systemctl start pgpool-II-$pg_family
  until $pg_home/bin/psql -h localhost -p 9999 -U "postgres" -c '\l'; do
  ...
  sudo systemctl stop pgpool-II-$pg_family
...
if [ ! -f "~/.backup_data" ]; then
  ...
  tar czf ~/backup-pgpool-etc.tar.gz /etc/pgpool-II-$pg_family/*
...
if [ ! -f "~/.nfs_configured" ] && [ $use_nfs -eq 1 ]; then
  ...
  if ping -c 1 nfssrv01 &> /dev/null; then
    ...
    sudo rsync -avr /etc/pgpool-II-$pg_family/ /mnt/
    sudo umount /mnt
    sudo rm -rf /opt/opennms/etc/*
    sudo rm -rf /var/opennms/*
    sudo rm -rf /etc/pgpool-II-$pg_family/*
  else
...
if [ ! -f "~/.drbd_configured" ] && [ $use_nfs -eq 0 ]; then
  ...
  sudo rsync -avr --delete /etc/pgpool-II-$pg_family/ /drbd/pgpool/etc/
  sudo rm -rf /opt/opennms/etc
  sudo rm -rf /var/opennms
  sudo rm -rf /etc/pgpool-II-$pg_family
  sudo ln -s /drbd/opennms/etc /opt/opennms/etc
  sudo ln -s /drbd/opennms/var /var/opennms
  sudo ln -s /drbd/pgpool/etc /etc/pgpool-II-$pg_family
