...
IMPORTANT: For _PostgreSQL 9.0_ to _9.3_, `max_replication_slots` cannot be used. In case it is required to use an older version, you should replace `max_replication_slots` with `wal_keep_segments = 5000`.
...
Also sudo should be enabled for `repmgr`:

[source, bash]
----
cat <<EOF > /etc/sudoers.d/postgres
Defaults:postgres !requiretty
postgres ALL = NOPASSWD: /bin/systemctl status postgresql-9.6, \
/bin/systemctl start postgresql-9.6, \
/bin/systemctl stop postgresql-9.6, \
/bin/systemctl reload postgresql-9.6, \
/bin/systemctl restart postgresql-9.6
EOF
...
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --force -v -d repmgr -p 5432 -U repmgr -R postgres standby clone pgdbsrv02
NOTICE: using provided configuration file "/etc/repmgr/9.6/repmgr.conf"
NOTICE: destination directory "/var/lib/pgsql/9.6/data" provided
INFO: connecting to source node
NOTICE: checking for available walsenders on source node (2 required)
INFO: sufficient walsenders available on source node (2 required)
WARNING: directory "/var/lib/pgsql/9.6/data" exists but is not empty
NOTICE: -F/--force provided - deleting existing data directory "/var/lib/pgsql/9.6/data"
INFO: replication slot "repmgr_slot_1" created on source node
NOTICE: starting backup (using pg_basebackup)...
HINT: this may take some time; consider using the -c/--fast-checkpoint option
INFO: executing:
  /usr/pgsql-9.6/bin/pg_basebackup -l "repmgr base backup"  -D /var/lib/pgsql/9.6/data -h postgresql2 -p 5432 -U repmgr -X stream -S repmgr_slot_1 
INFO: replication slot "repmgr_slot_1" aleady exists on upstream node 2
NOTICE: standby clone (using pg_basebackup) complete
NOTICE: you can now start your PostgreSQL server
HINT: for example: systemctl start postgresql-9.6
----
...
----
[root@pgdbsrv01 ~]# su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --force --rsync-only -v -d repmgr -p 5432 -U repmgr -R postgres standby clone pgdbsrv02
...
IMPORTANT: `--rsync-only` doesn't seem to work on repmgr 4.x.
