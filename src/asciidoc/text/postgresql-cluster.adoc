...
[source]
----
include::../files/postgresql-cluster/repmgr.master.conf[]
----
...
----
[root@pgdbsrv01 ~]$ sudo su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf -v master register
NOTICE: using provided configuration file "/etc/repmgr/9.6/repmgr.conf"
NOTICE: attempting to install extension "repmgr"
NOTICE: "repmgr" extension successfully installed
WARNING: get_primary_node_id(): no active primary found
...
[source]
----
include::../files/postgresql-cluster/repmgr.slave.conf[]
----
...
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -h pgdbsrv01 -U repmgr -d repmgr -f /etc/repmgr/9.6/repmgr.conf standby clone --dry-run
...
If no problems are reported, sync the configuration and contents of the primary with the standby:

[source, bash]
----
[root@pgdbsrv02 ~]$ sudo su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -h pgdbsrv01 -U repmgr -d repmgr -f /etc/repmgr/9.6/repmgr.conf -v standby clone
NOTICE: using provided configuration file "/etc/repmgr/9.6/repmgr.conf"
NOTICE: destination directory "/var/lib/pgsql/9.6/data" provided
INFO: connecting to source node
...
INFO: checking and correcting permissions on existing directory "/var/lib/pgsql/9.6/data"
INFO: replication slot "repmgr_slot_2" created on source node
NOTICE: starting backup (using pg_basebackup)...
HINT: this may take some time; consider using the -c/--fast-checkpoint option
INFO: executing:
  pg_basebackup -l "repmgr base backup"  -D /var/lib/pgsql/9.6/data -h psql01 -p 5432 -U repmgr -S repmgr_slot_2 --xlog-method=stream
INFO: replication slot "repmgr_slot_2" aleady exists on upstream node 1
...
----
[root@pgdbsrv02 ~]$ su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf -v standby register
INFO: connecting to local node "psql02" (ID: 2)
INFO: connecting to primary database
INFO: searching for primary node
INFO: checking if node 1 is primary
INFO: current primary node is 1
INFO: standby registration complete
NOTICE: standby node "psql02" (id: 2) successfully registered
-bash-4.1$ exit
logout
----
...
----+--------+---------+-----------+----------+----------+--------------------------------------
 1  | psql01 | primary | * running |          | default  | host=psql01 user=repmgr dbname=repmgr
 2  | psql02 | standby |   running | psql01   | default  | host=psql02 user=repmgr dbname=repmgr
----
