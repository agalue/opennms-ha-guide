...
[source, bash]
----
[root@pgdbsrv01 ~]# service postgresql-9.6 initdb
...
[source, bash]
----
[root@pgdbsrv01 ~]# /usr/pgsql-9.6/bin/postgresql96-setup initdb
Initializing database ... OK
----

Configure database access by editing `/var/lib/pgsql/9.6/data/pg_hba.conf`, and adding the following content:
...
Configure _PostgreSQL_ itself and streaming replication by editing `/var/lib/pgsql/9.6/data/postgresql.conf`, and updating the following attributes:
...
By default, the configuration directory for _repmgr_ is located at `/etc/repmgr/9.6/`.
...
[source, bash]
----
[root@pgdbsrv01 ~]$ chown postgres:postgres /etc/repmgr/9.6/repmgr.conf
----
...
[source, bash]
----
[root@pgdbsrv01 ~]# chkconfig postgresql-9.6 on
[root@pgdbsrv01 ~]# service postgresql-9.6 start
...
----
[root@pgdbsrv01 ~]$ sudo su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --verbose master register
[2016-06-17 12:42:11] [NOTICE] using configuration file "/etc/repmgr/9.6/repmgr.conf"
...
Configure _repmgr_ similar to the primary by creating `/etc/repmgr/9.6/repmgr.conf` with the following content:
...
[source, bash]
----
[root@pgdbsrv02 ~]$ chown postgres:postgres /etc/repmgr/9.6/repmgr.conf
----
...
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --verbose -D /var/lib/pgsql/9.6/data -d repmgr -p 5432 -U repmgr -R postgres standby clone pgdbsrv01
2016-06-17 12:47:17] [NOTICE] using configuration file "/etc/repmgr/9.6/repmgr.conf"
[2016-06-17 12:47:17] [NOTICE] destination directory '/var/lib/pgsql/9.6/data' provided
...
[2016-06-17 12:47:17] [INFO] creating directory "/var/lib/pgsql/9.6/data"...
...
[2016-06-17 12:47:17] [INFO] executing: '/usr/pgsql-9.6/bin/pg_basebackup -l "repmgr base backup"  -D /var/lib/pgsql/9.6/data -h pgdbsrv01 -p 5432 -U repmgr --xlog-method=stream'
...
[2016-06-17 12:47:17] [HINT] for example : pg_ctl -D /var/lib/pgsql/9.6/data start
...
In case you experience problems with the synchronization and you need to repeat this step, first delete the contents of the _PostgreSQL_ datadir (`rm -rf /var/lib/pgsql/9.6/data/*`) and try again.

On _RHEL/CentOS 6_:

[source, bash]
----
[root@pgdbsrv02 ~]# chkconfig postgresql-9.6 on
[root@pgdbsrv02 ~]# service postgresql-9.6 start
Starting postgresql-9.6 service:                           [  OK  ]
----
...
[source, bash]
----
[root@pgdbsrv02 ~]# systemctl enable postgresql-9.6
ln -s '/usr/lib/systemd/system/postgresql-9.6.service' '/etc/systemd/system/multi-user.target.wants/postgresql-9.6.service'
[root@pgdbsrv02 ~]# systemctl start postgresql-9.6
----
...
----
[root@pgdbsrv02 ~]$ su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --verbose standby register
[2016-06-17 12:50:17] [NOTICE] using configuration file "/etc/repmgr/9.6/repmgr.conf"
...
[source, bash]
----
[root@pgdbsrv01 ~]# su - postgres -c "/usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf cluster show"
...
[source, bash]
----
[root@onmssrv01 ~]# cd /etc/pgpool-II-96/
[root@onmssrv01 pgpool-II-96]# cp pgpool.conf.sample-stream pgpool.conf
----

Edit the following settings in the copied sample file `/etc/pgpool-II-96/pgpool.conf`:
...
failover_command = '/etc/pgpool-II-96/failover.sh %h %H'
...
IMPORTANT: For _RHEL/CentOS 6_, the `pid_file_name` must be `/var/run/pgpool-II-96/pgpool.pid`.
           For _RHEL/CentOS 7_ this is enforced through `pgpool-II-96.service` (the _systemd_ initialization script).
           That way, the content of `pgpool-II-96.conf` is the same for both operating systems.
...
[source, bash]
----
[root@onmssrv01 ~]$ chmod 0600 /etc/pgpool-II-96/pgpool.conf
[root@onmssrv01 ~]$ chown postgres:postgres /etc/pgpool-II-96/pgpool.conf
----
...
On the above settings, we specified that on failover, the script `failover.sh` should be executed, so we’ll need to create this file in `/etc/pgpool-II-96/failover.sh`:
...
[source, bash]
----
[root@onmssrv01 ~]$ chmod 0700 /etc/pgpool-II-96/failover.sh
[root@onmssrv01 ~]$ chown postgres:postgres /etc/pgpool-II-96/failover.sh
----

We also specified in the configuration that we want to use `pool_hba.conf`, so we need to add the credentials to the `/etc/pgpool-II-96/pool_hba.conf` file.
...
[source, bash]
----
[root@onmssrv01 ~]$ chmod 0600 /etc/pgpool-II-96/pool_hba.conf
[root@onmssrv01 ~]$ chown postgres:postgres /etc/pgpool-II-96/pool_hba.conf
----
...
[source, bash]
----
[root@onmssrv01 ~]# touch /etc/pgpool-II-96/pool_passwd
[root@onmssrv01 ~]# chmod 600 /etc/pgpool-II-96/pool_passwd
[root@onmssrv01 ~]# chown postgres:postgres /etc/pgpool-II-96/pool_passwd
----
...
[source, bash]
----
[root@onmssrv01 ~]# echo "postgres:e8a48653851e28c69d0506508fb27fc5" >> /etc/pgpool-II-96/pcp.conf
----
...
[source, bash]
----
[root@onmssrv01 ~]$ chmod 0600 /etc/pgpool-II-96/pcp.conf
[root@onmssrv01 ~]$ chown postgres:postgres /etc/pgpool-II-96/pcp.conf
----

At the end, you should have something like the following at `/etc/pgpool-II-96/`:
...
[source, bash]
----
[root@onmssrv01 ~]$ mkdir /var/log/pgpool-II-96
[root@onmssrv01 ~]$ chown postgres:postgres /var/log/pgpool-II-96
[root@onmssrv02 ~]$ mkdir /var/log/pgpool-II-96
[root@onmssrv02 ~]$ chown postgres:postgres /var/log/pgpool-II-96
----
...
[source, bash]
----
[root@onmssrv01 ~]# rsync -avr --delete /etc/pgpool-II-96/ onmssrv02:/etc/pgpool-II-96/
----
...
Jun 17 14:04:33 onmssrv01 pgpool: 2016-06-17 14:04:33: pid 32076: LOG:  Backend status file /var/log/pgpool-II-96/pgpool_status does not exist
...
[source, bash]
----
[root@onmssrv01 ~]# systemctl status pgpool-II-96
● pgpool-II-96.service - PGPool-II Middleware Between PostgreSQL Servers And PostgreSQL Database Clients
   Loaded: loaded (/usr/lib/systemd/system/pgpool-II-96.service; disabled; vendor preset: disabled)
 ...
 Main PID: 32076 (pgpool)
   CGroup: /system.slice/pgpool-II-96.service
           ├─32076 /usr/pgpool-9.6/bin/pgpool -f /etc/pgpool-II-96/pgpool.conf -n --discard-status
...
Jun 17 14:04:33 onmssrv01.local pgpool[32076]: 2016-06-17 14:04:33: pid 32076: LOG:  Backend status file /var/log/pgpool-II-96/... exist
...
Jun 17 14:04:33 onmssrv01.local pgpool[32076]: 2016-06-17 14:04:33: pid 32076: LOG:  Backend status file /var/log/pgpool-II-96/pgpool_status does not exist
Jun 17 14:04:33 onmssrv01.local pgpool[32076]: 2016-06-17 14:04:33: pid 32076: LOG:  Setting up socket for 0.0.0.0:9999
Jun 17 14:04:33 onmssrv01.local pgpool[32076]: 2016-06-17 14:04:33: pid 32076: LOG:  Setting up socket for :::9999
...
This should also work from another host that has network access to the _pgpool-II_ machine, assuming that host is has the appropriate permissions in /etc/pgpool-II-96/pool_hba.conf.
...
WARNING: Remember to repeat the connection test from _onmssrv02_. Be sure that /etc/pgpool-II-96 has been synched between both servers.
...
[source, bash]
----
[root@pgdbsrv01 ~]# service postgresql-9.6 stop
----
...
[source, bash]
----
[root@pgdbsrv01 ~]# systemctl stop postgresql-9.6
----
...
[source, bash]
----
[root@pgdbsrv02 ~]# su - postgres -c " /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf cluster show"
...
The failover script should add some useful information to `/var/log/pgpool-II-96/pgpool_failover.log`, when a failover happens:

[source, bash]
----
[root@onmssrv01 ~]# cat /var/log/pgpool-II-96/pgpool_failover.log
Fri Jun 17 14:44:49 EDT 2016
Failed node: pgdbsrv01, Promoting pgdbsrv02 ...
+ /usr/bin/ssh -T -l postgres pgdbsrv02 '/usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf standby promote 2>/dev/null 1>/dev/null <&-'
+ exit 0
----
...
(1) Perform a full synchronization, in other words, recreate the whole `/var/lib/pgsql/9.6/data` directory from scratch:

[source, bash]
----
[root@pgdbsrv01 ~]# su - postgres
-bash-4.1$ rm -rf 9.6/data/
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --verbose -D /var/lib/pgsql/9.4/data -d repmgr -p 5432 -U repmgr -R postgres standby clone pgdbsrv02
[2016-06-17 14:49:37] [NOTICE] using configuration file "/etc/repmgr/9.6/repmgr.conf"
...
[2016-06-17 14:49:37] [INFO] executing: '/usr/pgsql-9.6/bin/pg_basebackup -l "repmgr base backup"  -D /var/lib/pgsql/9.4/data -h pgdbsrv02 -p 5432 -U repmgr --xlog-method=stream'
...
----
[root@pgdbsrv01 ~]# su - postgres
-bash-4.1$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --verbose --force --rsync-only -D /var/lib/pgsql/9.6/data -d repmgr -p 5432 -U repmgr -R postgres standby clone pgdbsrv02
[2016-06-17 14:53:20] [NOTICE] using configuration file "/etc/repmgr/9.6/repmgr.conf"
[2016-06-17 14:53:20] [NOTICE] destination directory '/var/lib/pgsql/9.6/data' provided
...
[2016-06-17 14:53:20] [INFO] creating directory "/var/lib/pgsql/9.6/data"...
[2016-06-17 14:53:20] [NOTICE] starting backup (using rsync)...
[2016-06-17 14:53:21] [INFO] standby clone: master data directory '/var/lib/pgsql/9.6/data'
[2016-06-17 14:53:21] [INFO] rsync command line: 'rsync --archive --checksum --compress --progress --rsh=ssh --delete --checksum --exclude=postmaster.pid --exclude=postmaster.opts --exclude=global/pg_control --exclude=recovery.conf --exclude=recovery.done --exclude=postgresql.auto.conf.tmp --exclude=pgsql_tmp* --exclude=pg_xlog/* --exclude=pg_log/* --exclude=pg_stat_tmp/* postgres@pgdbsrv02:/var/lib/pgsql/9.6/data/* /var/lib/pgsql/9.6/data'
receiving incremental file list
.....
sent 21861 bytes  received 3528512 bytes  2366915.33 bytes/sec
total size is 30168542  speedup is 8.50
[2016-06-17 14:53:22] [INFO] read_backup_label: parsing backup label file '/var/lib/pgsql/9.6/data/backup_label'
Label: repmgr_standby_clone_1466189601; file: 000000020000000000000009
[2016-06-17 14:53:22] [INFO] standby clone: local control file '/var/lib/pgsql/9.6/data/global'
[2016-06-17 14:53:22] [INFO] standby clone: master control file '/var/lib/pgsql/9.6/data/global/pg_control'
[2016-06-17 14:53:22] [INFO] rsync command line: 'rsync --archive --checksum --compress --progress --rsh=ssh --delete --checksum postgres@pgdbsrv02:/var/lib/pgsql/9.6/data/global/pg_control /var/lib/pgsql/9.6/data/global'
receiving incremental file list
pg_control
...
[2016-06-17 14:53:23] [HINT] for example : pg_ctl -D /var/lib/pgsql/9.6/data start
...
[source, bash]
----
[root@pgdbsrv01 ~]# sudo su - postgres -c " /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf cluster show"
...
For the `pcp_attach_node` command, it is required to use the authentication credentials defined on `/etc/pgpool-II-96/pcp.conf`. Also, it is required to know the node_id of the node you want to re-attach. The "show pool_nodes" command will give you all the information (including the `node_id`).
...
[source, bash]
----
[root@pgdbsrv02 ~]# service postgresql-9.6 stop
----
...
[source, bash]
----
[root@pgdbsrv02 ~]# systemctl stop postgresql-9.6
----
...
----
[root@pgdbsrv02 ~]# sudo su - postgres
-bash-4.2$ /usr/pgsql-9.6/bin/repmgr -f /etc/repmgr/9.6/repmgr.conf --verbose --force --rsync-only -D /var/lib/pgsql/9.6/data -d repmgr -p 5432 -U repmgr -R postgres standby clone pgdbsrv01
----

Then, start the _postgresql-9.6_ service.
