...
This tutorial was designed for _Meridian_, but should work without issues for _OpenNMS Horizon_.
...
The _NFS_ server must have the following content on its `/etc/exports` file:
...
The above paths must be accessible form the _OpenNMS_ servers.
...
We just make sure that the local copy of `/opt/opennms/etc`, `/var/opennms` and `/etc/pgpool-II/` are synchronized with the _NFS_ server:
...
Remove all the content form the directories that will be mounted through _NFS_:
...
IMPORTANT: Make sure that the permissions on `/etc/pgpool-II/` are correct.
...
As a workaround, do not use the cluster resource associated with `/etc/pgpool-II/`, synchronize the `/etc/pgpool-II/` directory back to each _OpenNMS_ server, and fix the permissions.
...
IMPORTANT: Do not declare the external filesystem on `/etc/fstab`, the mount/umount operation of the shared filesystems will be managed through the cluster services, not the operating system.
