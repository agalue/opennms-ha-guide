...
IMPORTANT: Make sure to install the above package on all the servers: `onmssrv01`, `onmssrv02`, `pgdbsrv01` and `pgdbsrv02`.
...
For this reason, install the following packages on each of the _pgpoo-II/OpenNMS_ servers:
...
IMPORTANT: Repeat all the above commands on `onmssrv02`
...
[source, bash]
----
[root@onmssrv01 ~]# curl -v -L -H "Cookie: oraclelicense=accept-securebackup-cookie" http://download.oracle.com/otn-pub/java/jdk/8u91-b14/jdk-8u91-linux-x64.rpm -o /tmp/jdk-8u91-linux-x64.rpm
[root@onmssrv01 ~]# yum install -y /tmp/jdk-8u91-linux-x64.rpm
----
...
[source, bash]
----
[root@onmssrv01 ~]# yum install opennms-core opennms-webapp-jetty 'perl(LWP)' 'perl(XML::Twig)' -y
----
...
[source, bash]
----
[root@onmssrv01 ~]# yum install meridian-core meridian-webapp-jetty 'perl(LWP)' 'perl(XML::Twig)' -y
----
...
[source, bash]
----
[root@onmssrv01 ~]# yum install jrrd rrdtool -y
----

For `Horizon 17` or newer:
[source, bash]
----
[root@onmssrv01 ~]# yum install jrrd2 rrdtool -y
----

It is recommended to install the `haveged` package on all the _OpenNMS_ servers. This service reduces the startup time of the _OpenNMS_ application.
...
[source, bash]
----
[root@onmssrv01 ~]# yum install https://dl.fedoraproject.org/pub/epel/epel-release-latest-6.noarch.rpm -y
[root@onmssrv01 ~]# yum install haveged -y
[root@onmssrv01 ~]# chkconfig haveged on
[root@onmssrv01 ~]# service haveged start
----
...
[source, bash]
----
[root@onmssrv01 ~]# yum install https://dl.fedoraproject.org/pub/epel/epel-release-latest-7.noarch.rpm -y
[root@onmssrv01 ~]# yum install haveged -y
[root@onmssrv01 ~]# systemctl enable haveged
[root@onmssrv01 ~]# systemctl start haveged
----

IMPORTANT: Do not make _OpenNMS_ to start with the operating system.  Instead it will be managed by the cluster services.
...
On each _PostgreSQL_ server, create a script called `cleanup.sh` at `/var/lib/psql/9.5/` with the following content:
...
[source, bash]
----
[root@pgdbsrv01 ~]# chmod 0700 /var/lib/pgsql/9.5/cleanup.sh
[root@pgdbsrv01 ~]# chown postgres:postgres /var/lib/pgsql/9.5/cleanup.sh
----

IMPORTANT: Do not forget to update `onmssrv02` and `pgdbsrv02`.
...
[source, bash]
----
chmod 06444 /lib/systemd/system/pgpool-II-95.service
----
...
You should also see _drbd_ if that was configured as well.
...
Enable the _drbd_ service (if applies):

[source, bash]
----
[root@onmssrv01 ~]# firewall-cmd --permanent --add-service=drbd
success
[root@onmssrv01 ~]# firewall-cmd --add-service=drbd
success
----
...
IMPORTANT: Repeat all the process on the standby _OpenNMS_ server (_onmssrv02_).

On each of the _PostgreSQL_ servers, execute the following commands:
...
IMPORTANT: Repeat the process on the standby _PostgreSQL_ server (_pgdbsrv02_).
