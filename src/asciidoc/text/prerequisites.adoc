...
In order to use _CentOS 7_, replace the value of the `config.vm.box` attribute from `bento/centos-6.6` to `bento/centos-7.1`.
...
Because this solution required the usage of _PostgreSQL 9.4_, _repmgr 3.0_ and _pgpool-II 3.4_, the  _PGDG_ repository needs to be added.

The _PGDG_ provides packages for _pgpool-II_, but a different repository will be used from additional dependency issues with the _RPM_ provided on the _PGDG_ repository.
...
Information about the _pgpool-II_ _YUM_ repository is provided here:

http://www.pgpool.net/mediawiki/index.php/Yum_Repository

Add the _PGDG_ repo on all nodes:
...
As previously mentioned, use the following repository for _pgpool-II_.  This repository must be installed only on the _Pgpool-II_ server, which are the _OpenNMS_ servers in this case.
...
To avoid version conflicts between the two versions of _pgpool-II_ and _libmemcached_, edit the _PGDG_ repository, `/etc/yum.repos.d/pgdg-94-centos.repo` file.  Exclude _libmemcached_ by adding the following line at the end of the section called `pgdb94`.  This is done on the _OpenNMS_ servers:
...
Forgetting this step will result in errors when executing `yum update`.
...
To use _RRDtool_ with _Horizon_ and _RHEL/CentOS 6_, it is recommended to download the `rrdtool` and `rrdtool-perl` packages (version 1.4.7) from http://packages.express.org/rrdtool/, instead of using the packages from the distribution.
...
Edit `/usr/lib/tmpfiles.d/pgpool-II-pg94.conf` and make sure the contents look like the following:
...
Once the _SSH_ keys are aquired, add the generated public keys from all hosts to `/var/lib/pgsql/.ssh/authorized_keys`.
